...
//
// Missed symbols back off exponentially (2min → 4 → 8 → … capped at 60min)
// and stay out of the hot custom batch until their next retry. Due symbols
// share at most one extra quotes/latest call per tick.
...
export function getCmcCachePath(userDataPath?: string) {
  ...
  // Lazy require so unit tests do not construct Electron Conf userData folders.
  require('../../identity')
