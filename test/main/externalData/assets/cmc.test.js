...
describe('CMC price feed cache and concurrency', () => {
  ...
  it('hydrates main.rates-bound updates from disk before the first network poll', async () => {
    ...
    feed = createCmcPriceFeed(createStore(), onUpdates, { cachePath })
    feed.start()
  ...
  it('keeps not-due failed symbols out of the hot batch and retries them separately when due', async () => {
    ...
    const started = pendingFetch()
    feed = createCmcPriceFeed(createStore(), jest.fn(), { cachePath })
    feed.updateSubscription([1, 8453], ACCOUNT)
    feed.start()
    await jest.advanceTimersByTimeAsync(1000)
  ...
  it('catches up within seconds when more chains appear during a partial first poll', async () => {
    ...
    const started = pendingFetch()
    feed = createCmcPriceFeed(store, jest.fn(), { cachePath })
  ...
  it('writes cmc-rates.json as soon as the first quotes land', async () => {
    ...
    const onUpdates = jest.fn()
    feed = createCmcPriceFeed(store, onUpdates, { cachePath })
    feed.updateSubscription([1, 8453], ACCOUNT)
    feed.start()
    await jest.advanceTimersByTimeAsync(FIRST_POLL_DELAY_MS)
  ...
  it.each([401, 403])('keeps last-good cache and does not fall back to Pylon on %s', async (status) => {
    saveCmcCache(cachePath, {
      version: 1,
      updatedAt: 1,
      rates: {
    ...
    fetchWithTimeout.mockResolvedValue({
      ok: false,
      status,
      json: async () => ({ status: { error_code: 1000, error_message: 'API key invalid' } })
    })
    ...
    expect(loadCmcCache(cachePath).rates['n:1'].usd).toBe(2460)
    ...
    expect(loadCmcCache(cachePath).rates['n:1'].usd).toBe(2460)
    expect(JSON.stringify(loadCmcCache(cachePath))).not.toMatch(/test-cmc-key|API key invalid/)
